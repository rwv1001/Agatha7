  ...
  class << self
    ...
    def attempt_lock(table_name, record_id, user_id)
      ...
      if existing_lock
        ...
        if existing_lock.user_id == user_id
          ...
          {locked: true, message: "Lock refreshed"}
        elsif lock_expired?(existing_lock)
          # Check if lock has expired
          existing_lock.update(in_use: false)
          create_lock(table_name, record_id, user_id)
          {locked: true, message: "Previous lock expired, new lock acquired"}
        ...
        create_lock(table_name, record_id, user_id)
        {locked: true, message: "Lock acquired successfully"}
      end
    end
    ...
    def release_lock(table_name, record_id, user_id)
      ...
      if lock
        ...
        Rails.logger.debug "RecordLockService: Released lock for #{table_name}##{record_id} by user #{user_id}"
        {unlocked: true, message: "Lock released successfully"}
      else
        {unlocked: false, message: "No active lock found for this user"}
      end
    end
    ...
    def cleanup_expired_locks
      ...
      expired_locks.find_each do |lock|
        ...
        Rails.logger.debug "RecordLockService: Expired lock for #{lock.table_name}##{lock.record_id} by user #{lock.user_id}"
      end
    ...
    def create_lock(table_name, record_id, user_id)
      ...
      Rails.logger.debug "RecordLockService: Created lock for #{table_name}##{record_id} by user #{user_id}"
    end
