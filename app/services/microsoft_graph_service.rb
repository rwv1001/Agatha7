...
class MicrosoftGraphService
  ...
  def send_email(agatha_email, to_email)
    Rails.logger.info("MicrosoftGraphService: Sending email via Microsoft Graph API")
    Rails.logger.info("To: #{to_email}, Subject: #{agatha_email.subject}")
    ...
    begin
      ...
      if response.code == '202'
        Rails.logger.info("MicrosoftGraphService: Email sent successfully")
        return { success: true, message: "Email sent via Microsoft Graph" }
      else
