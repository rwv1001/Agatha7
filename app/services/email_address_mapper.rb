class EmailAddressMapper
  ...
  def self.map_from_address(original_from_address)
    ...
    if cleaned_address.end_with?("@#{DISPLAY_DOMAIN}")
      validation_email = "#{local_part}@#{OWNED_DOMAIN}"
    elsif cleaned_address.end_with?("@#{OWNED_DOMAIN}")
      validation_email = cleaned_address
    else
  ...
  def self.log_graph_api_error(operation, response)
    error_body = JSON.parse(response.body) rescue { 'error' => { 'code' => 'ParseError', 'message' => response.body } }
    ...
    Rails.cache.fetch("graph_user_mapping_#{target_email}", expires_in: 30.minutes) do
      begin
        ...
        if user
          ...
          Rails.logger.info "EmailAddressMapper: Found sender #{sender_email} for #{target_email}"
          sender_email
        else
  ...
  def self.extract_sender_email_from_user(user)
    ...
    english_proxies = proxy_addresses.select { |addr| 
      addr.match(/^SMTP:.*@#{Regexp.escape(SENDER_DOMAIN)}$/i) || 
      addr.match(/^smtp:.*@#{Regexp.escape(SENDER_DOMAIN)}$/i)
    }
  ...
  def self.fetch_sender_domain_accounts
    ...
    Rails.cache.fetch("sender_domain_accounts", expires_in: 1.hour) do
      begin
        ...
        Rails.logger.info "EmailAddressMapper: Fetched #{sender_emails.length} sender domain accounts"
        sender_emails
      ...
      begin
        ...
        valid_emails = users.select { |user| 
          ...
          (user['proxyAddresses'] || []).any? { |addr| 
            addr.match(/^SMTP:.*@#{Regexp.escape(OWNED_DOMAIN)}$/i) ||
            addr.match(/^smtp:.*@#{Regexp.escape(OWNED_DOMAIN)}$/i)
          }
        }.map { |user| 
        ...
        Rails.logger.info "EmailAddressMapper: Fetched #{valid_emails.length} valid accounts from Microsoft Graph"
        valid_emails
  ...
  def self.test_email_mapping(email_address)
    ...
    if email_address.end_with?("@#{DISPLAY_DOMAIN}")
      step1_studium_email = "#{local_part}@#{OWNED_DOMAIN}"
    else
      step1_studium_email = cleaned_email
    end
