class SearchTableChannel < ApplicationCable::Channel
  def subscribed
    ...
    Rails.logger.debug "User #{current_user_id} subscribed to search table updates"
  end

  def unsubscribed
    Rails.logger.debug "User #{current_user_id} unsubscribed from search table updates"
  end
  ...
  def join_table(data)
    ...
    if table_name.present?
      ...
      Rails.logger.debug "User #{current_user_id} joined #{table_name} updates"
    end
  end
