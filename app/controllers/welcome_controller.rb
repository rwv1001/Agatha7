...
class WelcomeController < ApplicationController
    ...
    for external_filter in external_filters
      ...
      if params.has_key?(num_elements_str)
        ...
        for elt_id in (0..(num_elts - 1))
          external_filter_elt = if filter_count >= old_external_filter_elt_count
            ExternalFilterValue.new
          else
            old_external_filter_elts[filter_count]
          end
          ...
          group_id = if params.has_key?(group_name_str)
  ...
  def new
    ...
    if class_name == "EmailTemplate"
      ...
      new_obj.body = "Dear <%= person.salutation %>,<br><br>Put your email content here "
    end
    ...
    # ModelDependencyService.send_data_invalidation_for_update(class_name, new_obj, nil, id)
  ...
  def suggest_tutorial
    ...
    if willing_tutor.length > 0
      ...
      tutorial_schedule.person_id = if first_suggestion
        if first_suggestion[0].to_i == SearchController::NOT_SET && first_suggestion.length > 1
          first_suggestion[1].to_i
        else
          first_suggestion[0].to_i
        end
      else
        SearchController::NOT_SET
      end
    ...
    if old_term_id == SearchController::NOT_SET
      ...
      tutorial_schedule.term_id = if !suggested_term.nil? && suggested_term == SearchController::NOT_SET
        suggested_term.id
      else
        Term.last.id
      end
    ...
    tutorial_schedule.number_of_tutorials = if old_number_of_tutorials == 0 && db_tutorial_schedules.length > 0
      db_tutorial_schedules[0].number_of_tutorials
    else
      old_number_of_tutorials
    end
  ...
  def suggest_lecture
    ...
    if willing_lecturers.length > 0
      ...
      first_suggestion = previous_suggestions.scan(/\d+/)
      lecture.person_id = if first_suggestion
        if first_suggestion[0].to_i == SearchController::NOT_SET && first_suggestion.length > 1
          first_suggestion[1].to_i
        else
          first_suggestion[0].to_i
    ...
    if old_term_id == SearchController::NOT_SET
      suggested_term = session[:current_term]
      lecture.term_id = if !suggested_term.nil? && suggested_term == SearchController::NOT_SET
        suggested_term.id
      else
        Term.last.id
      end
    ...
    lecture.number_of_classes = if old_classes == 0 && db_lectures.length > 0
      db_lectures[0].number_of_classes
    else
      old_classes
    end
    old_lectures = params[:number_of_lectures].to_i
    lecture.number_of_lectures = if old_lectures == 0 && db_lectures.length > 0
      db_lectures[0].number_of_lectures
    else
      old_lectures
    end
  ...
  def multi_update(ids, class_name)
    ...
    if ids.nil? || ids.length == 0
      ...
      params.each do |key, value|
        if key&.match?(Regexp.new("^mi_edit_#{class_name}_"))
        ...
        elsif key&.match?(Regexp.new("^mt_edit_#{class_name}_"))
          if value.length > 0
            if field_str.length > 0
              field_str << ", "
            end
        ...
        ids.each do |id|
          ...
          email_addresses.each do |email_address|
            if /@/.match?(email_address)
              ...
              to_email = if test_flag == 1
                me.email
              else
                email_address
              end
              Rails.logger.debug("send_emails_routine: email to address is #{to_email}!")
              Rails.logger.debug("send_emails_routine: email from address is #{agatha_email.from_email}")
              Rails.logger.debug("send_emails_routine: email subject is #{agatha_email.subject}")
              begin
                # Send email and check for Microsoft Graph result
                mail_result = AgathaMailer.with(agatha_email: agatha_email, to_email: to_email).email.deliver_now
                Rails.logger.debug("send_emails_routine: mail_result = #{mail_result.inspect}")

                # Get the Microsoft Graph result from the class variable
                microsoft_graph_result = AgathaMailer.get_last_microsoft_graph_result
                Rails.logger.debug("send_emails_routine: microsoft_graph_result = #{microsoft_graph_result.inspect}")
          ...
          agatha_email.save
          if agatha_email.sent
            sent_ids << id
          end
        end

        # Broadcast ActionCable update to refresh email table after sending
        if has_emails > 0
          affected_relationships = []
          ...
          affected_relationships << {
            ...
            operation: "update",
            ids: sent_ids,
            reason: "Email sent status updated",
            source_operation: "send_emails"
          }
          ActionCable.server.broadcast("search_table_updates", {
            action: "data_invalidation",
            triggered_by: {
  ...
  def send_emails(ids)
    ...
    respond_to do |format|
      format.js { render "send_emails", locals: {status_val: status_val} }
    end
  end
    ...
    if ids.nil? || ids.length == 0
      ...
      if template.course_dependency && course_id == SearchController::NOT_SET
        ...
        ids.each do |id|
          ...
          body_str = if template.ruby_header.length > 0
            template.ruby_header + template.body
          else
            template.body
          end
  ...
  def assign_tutor(ids)
    ...
    if ids.nil? || ids.length == 0
      ...
      success_str = if num_updates == 0
        "No tutor updates were made. "
      elsif num_updates == 1
        "One tutor update was made. "
      else
        "#{num_updates} tutor updates were made. "
      end
    end
    @search_ctls = session[:search_ctls]
  ...
  def make_willing_lecturer(ids)
    ...
    if !ids.nil? && ids.length > 0
      ...
      alert_str = if new_willing == 0
        "No willing lecturer entries were added to the database. "
      elsif new_willing == 1
        "1 willing lecturer entry was added to the database. "
      else
        "#{new_willing} willing lecturer entries were added to the database. "
      end
  ...
  def make_willing_tutor(ids)
    ...
    if !ids.nil? && ids.length > 0
      ...
      alert_str = if new_willing == 0
        "No willing tutor entries were added to the database. "
      elsif new_willing == 1
        "1 willing tutor entry was added to the database. "
      else
        "#{new_willing} willing tutor entries were added to the database. "
      end
      if already_willing == 1
  ...
  def make_attendee(lecture_ids)
    ...
    if lecture_ids.nil? || lecture_ids.length == 0
      ...
      success_str = if non_present_num == 0
        "Attendees were updated, although no new lecture was attended because lecture in the selected list was already attended the person. "
      elsif non_present_num == 1
        "The person was added to one lecture attendee list. "
      else
        "The person was added to #{non_present_num} lecture attendee lists. "
      end
  ...
  def add_to_lectures(people_ids)
    ...
    if people_ids.nil? || people_ids.length == 0
      ...
      success_str = if non_present_num == 0
        "Attendees were updated, although no one was added because everyone in the selected list was already attending the lecture course. "
      elsif non_present_num == 1
        "One person was added to the lecture attendee list. "
      else
        "#{non_present_num} people were added to the lecture attendee list. "
      end
      if non_present_num != 0
  ...
  def update_formats
    ...
    for table_object in @format_controller.table_objects
      ...
      for i in (0..(new_fields_count - 1))
        format_elt = if i >= old_fields_count
          FormatElement.new
        else
          old_fields[i]
        end
  ...
  def div_test
    ...
    lecture_schedules = Lecture.find_by_sql("SELECT * FROM lectures WHERE course_id = #{course.id} AND term_id = #{term.id}")
    if lecture_schedules.length > 0 then lecture_id = lecture_schedules[0].id
                                         exam_attendees = Attendee.find_by_sql("SELECT *, a1.first_name || ' ' || a1.second_name AS student_name FROM attendees a0 INNER JOIN people a1 ON a1.id=a0.person_id AND a0.lecture_id = #{lecture_id} AND a0.examined= true ORDER BY student_name")
                                         non_exam_attendees = Attendee.find_by_sql("SELECT *, a1.first_name || ' ' || a1.second_name AS student_name FROM attendees a0 INNER JOIN people a1 ON a1.id=a0.person_id AND a0.lecture_id = #{lecture_id} AND a0.examined= false ORDER BY student_name")
    else
      exam_attendees = []
      non_exam_attendees = []
    end
  ...
  def import_csv
    ...
    for people_csv in people_csvs
      ...
      if entry_year.nil?
        ...
        person.entry_term_id = if entry_terms.length > 0
          entry_terms[0].id
        else
          SearchController::NOT_SET
        end
      ...
      for i in (0..1)
        ...
        if id != 0 && !id.nil?
          ...
          elsif institution = PersonCsv.find(id)
        ...
        person.institution_id = if new_ids[0] != 0
          new_ids[0]
        else
          @not_set_value
        end
        person.religious_house_id = if new_ids[1] != 0
          new_ids[1]
        else
          @not_set_value
        end
    ...
    for csv_day in csv_days
      ...
      day.sunday = /Sunday/.match?(csv_day.long_name) || false
      day.monday = /Monday/.match?(csv_day.long_name) || false
      day.tuesday = /Tuesday/.match?(csv_day.long_name) || false
      day.wednesday = /Wednesday/.match?(csv_day.long_name) || false
      day.thursday = /Thursday/.match?(csv_day.long_name) || false
      day.friday = /Friday/.match?(csv_day.long_name) || false
      day.saturday = /Saturday/.match?(csv_day.long_name) || false
    ...
    for csv_lecture in csv_lectures
      ...
      if !course_index.nil? && !term_index.nil? && !tutor_index.nil? && !day_index.nil?
        ...
        if /12:13/.match?(csv_lecture.lecture_time)
          x = 1
        else
    ...
    for csv_tutorial in csv_tutorials
      ...
      if !student_index.nil? && !course_index.nil? && !term_index.nil? && !tutor_index.nil?
        ...
        tutorial_schedule.number_of_tutorial_hours = if !tutorial_hours.nil? && tutorial_hours != 0
          tutorial_hours
        else
          0
        end
    ...
    confirmed_lecturer_id = (confirmed_lecturer_groups.length > 0) ? confirmed_lecturer_groups[0].id : 0
    confirmed_tutor_id = (confirmed_tutor_groups.length > 0) ? confirmed_tutor_groups[0].id : 0
    language_group_id = (language_groups.length > 0) ? language_groups[0].id : 0
