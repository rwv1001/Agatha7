class FormatController
  ...
  def self.FormatElements(table, user_id)
    ...
    Rails.logger.debug("FormatElements: before eval(#{sql_str})")
    ...
    if results.length == 0
      ...
      Rails.logger.debug("FormatElements: v2 before eval(#{sql_str})")
      results = eval(sql_str)
    end
    format_elements_array = []
    if results.length == 0
    ...
    FormatElts.new(table, user_id, format_elements_array)
  end
