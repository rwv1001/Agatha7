module EditHelper
  ...
  def edit_helper(table_name, readonly_fields)
    ...
    allow_edit = if !record_present || open_record.user_id == @user_id
      true; # will end up here only if close record when disconnected from the database - the session will be new but record still flagged as open in open_record table.
    else
      false
    end
      ...
      if object.nil? # 2
        ...
        if field_name.strip.length != 0
          ...
          if save_ok  # 4
            ...
            ModelDependencyService.send_data_invalidation_for_update(@table_name, object, attribute1.name, id, session[:user_id])
          ...
          if save_ok # 4
            ...
            # page.replace_html("#{@table_name}_#{field_name}", :partial => "shared/edit_cell", :object => edit_cell);
              ...
              if attribute.foreign_key.length > 0 # 7
                @filter_controller = FilterController.new(@search_ctls, @table_name, @user_id)
              elsif !body_value.nil? && body_value.length != 0 # 7
                update_parent = true
                edit_cell2 = EditCell.new(attribute, object, @table_name, @filter_controller, update_parent, read_only_flag) # 8
