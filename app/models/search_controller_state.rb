class SearchControllerState < ApplicationRecord
  ...
  attribute :controller_data, :json
  attribute :filters_data, :json
  attribute :search_direction, :json
  attribute :search_order, :json
  ...
  class << self
    def save_search_controller(user_id, table_name, search_controller)
      ...
      begin
        ...
        Rails.logger.debug "💾 Saved SearchController state for user #{user_id}, table #{table_name}"
        true
      rescue => e
    ...
    def load_search_controller_data(user_id, table_name)
      ...
      begin
        ...
        if state_record
          ...
          Rails.logger.debug "🔄 Loaded SearchController state for user #{user_id}, table #{table_name}"
          {
          ...
          Rails.logger.debug "🔍 No saved state found for user #{user_id}, table #{table_name}"
          nil
        end
      rescue => e
    ...
    def cleanup_old_states(days_old = 30)
      ...
      Rails.logger.debug "🧹 Cleaned up #{deleted_count} old SearchController states"
      deleted_count
    end
  end
