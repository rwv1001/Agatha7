class SearchPreference < ApplicationRecord
  ...
  def self.save_search_direction(user_id, table_name, direction_array)
    Rails.logger.info "🔧 SearchPreference.save_search_direction called with: user_id=#{user_id}, table_name=#{table_name}, direction=#{direction_array.inspect}"
    ...
    Rails.logger.info "🔧 Found/created preference: id=#{preference.id}, new_record=#{preference.new_record?}"
    
    preference.search_direction = direction_array
    
    Rails.logger.info "🔧 About to save preference with value: #{preference.preference_value}"
    
    if preference.save!
      Rails.logger.info "💾 ********* SUCCESS! Saved search direction to database at #{Time.now}: #{table_name} => #{direction_array}"
      return true
    else
  ...
  def self.load_search_direction(user_id, table_name)
    ...
    if preference
      ...
      Rails.logger.info "🔄 Loaded search direction from database: #{table_name} => #{direction}"
      direction
    else
      Rails.logger.info "🆕 No saved search direction found for: #{table_name}"
      nil
    end
  end
end
