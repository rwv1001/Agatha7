class ExternalFilterElement
  ...
  def MemberSelection
    @external_filter.MemberSelection(@group_id, @member_id)
  end
end
...
class ExternalFilter
  ...
  def initialize(class_name_, header_, where_str_, argument_class_, group_class_, argument_selector_str_, allow_multiple_arguments_, group_selector_)
    ...
    @group_selector_str = if group_selector_
      "Group.find_by_sql(\"SELECT a0.id, a0.group_name FROM groups a0 WHERE a0.table_name ='#{@argument_class.tableize}' AND (a0.owner_id = user_id_ OR administrator_ OR ((SELECT COUNT(*) FROM group_users a2 WHERE a2.user_id = user_id_ AND a2.group_id = a0.readers_id) > 0 AND a0.private = FALSE) ) ORDER BY a0.group_name asc\")"
    else
      ""
    end
  ...
  def Update # reads from DB to populate current_arguments
    sql_str = "ExternalFilterValue.find_by_sql(\"SELECT id, member_id, group_id FROM external_filter_values WHERE (user_id = #{@user_id} AND filter_id = #{@id} AND table_name = '#{class_name.tableize}' AND in_use = true ) ORDER BY id asc\")"
  ...
  def GroupSelection
    ...
    if group_selector_str.length > 0
      ...
      Rails.logger.debug("GroupSelection, @group_selector_str = #{@group_selector_str}")
  ...
  def MemberSelection(group_id, member_id)
    ...
    Rails.logger.debug("MemberSelection @argument_selector_str = #{@argument_selector_str}")
    eval(@argument_selector_str)
  end
end
